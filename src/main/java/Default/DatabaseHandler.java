...
public class DatabaseHandler {
    ...
    public static void initialiseDatabase(String adminUser, String adminPass) throws SQLException {
        ...
        statement.addBatch("CREATE TABLE IF NOT EXISTS sentences (Hash VARCHAR(32) NOT NULL PRIMARY KEY, Sentence TEXT NOT NULL, Occurrences INT UNSIGNED DEFAULT 0 NOT NULL, Documents INT UNSIGNED DEFAULT 0 NOT NULL, Blacklisted BIT DEFAULT 0, INDEX IDX_Occurrences(Occurrences), INDEX IDX_Documents(Documents));");
        statement.addBatch("CREATE TABLE IF NOT EXISTS newsarticles (ID INT UNSIGNED AUTO_INCREMENT NOT NULL PRIMARY KEY, Symbol VARCHAR(7) NOT NULL, Headline TEXT NOT NULL, Description TEXT, Content LONGTEXT, Published DATETIME NOT NULL, PublishedDate DATE, URL TEXT, Blacklisted BIT DEFAULT 0 NOT NULL, Redirected BIT DEFAULT 0 NOT NULL, Duplicate BIT DEFAULT 0 NOT NULL, Enumerated BIT DEFAULT 0 NOT NULL, Tokenised BIT DEFAULT 0 NOT NULL, Processed BIT DEFAULT 0 NOT NULL, Mood DOUBLE UNSIGNED DEFAULT 0.5, FOREIGN KEY (Symbol) REFERENCES indices(Symbol), INDEX IDX_Published(Published), INDEX IDX_PublishedDate(PublishedDate), INDEX IDX_Symbol_PublishedDate(Symbol, PublishedDate), INDEX IDX_Symbol_Published(Symbol, Published));");
        statement.addBatch("CREATE TRIGGER IF NOT EXISTS date_trigger BEFORE INSERT ON newsarticles FOR EACH ROW SET NEW.PublishedDate = DATE(NEW.Published);");
